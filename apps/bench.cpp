...
int main(int argc, char* argv[]) {
    try {
        ...
        /*
        H5::H5File interpFile(interpPath, H5F_ACC_RDONLY);
        H5::DataSet dataset = interpFile.openDataSet("profiles");
        H5::DataSpace filespace = dataset.getSpace();

        // Define the "hyperslab" (the slice at [1, 1, :])
        hsize_t offset[3] = {6, 3, 0};
        hsize_t count[3]  = {1, 1, (hsize_t)target_depths_mm.size()};
        filespace.selectHyperslab(H5S_SELECT_SET, count, offset);

        // Define memory space for the single profile
        hsize_t memdims[1] = {(hsize_t)target_depths_mm.size()};
        H5::DataSpace memspace(1, memdims);

        std::vector<double> prof(target_depths_mm.size());
        dataset.read(prof.data(), H5::PredType::NATIVE_DOUBLE, memspace, filespace);
        */
        ...
        try {
            ...
            std::string outPath =
                "simulation_results_" + s + "MeV_" + std::to_string(n_particles) + ".h5";
            H5::H5File outFile(outPath, H5F_ACC_TRUNC);

            /*
            /////////// for debug
            std::vector<double> dE_dx_MeV_mm = dE_dx;
            double inv_conv = 1.0 / conv_factor2;

            for (double &val : dE_dx_MeV_mm) {
                val *= inv_conv;
            }

            // 2. Save this converted vector to HDF5
            hsize_t dedx_matrix_dims[2] = {(hsize_t)out_Nx, (hsize_t)n_particles};
            H5::DataSpace dedx_matrix_space(2, dedx_matrix_dims);

            H5::DataSet dedx_matrix_ds = outFile.createDataSet("dE_dx_MeV_mm",
            H5::PredType::NATIVE_DOUBLE, dedx_matrix_space);
            dedx_matrix_ds.write(dE_dx_MeV_mm.data(), H5::PredType::NATIVE_DOUBLE);
            /////////////////
            */
