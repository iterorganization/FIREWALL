...
double Material::getK(double T) const {
    ...
    double Tm = 3695.0;
    if (T <= Tm) {
        return 149.441 + (3.866e6) / (T * T) - (45.466e-3) * T + (13.193e-6) * (T * T) -
               (1.484e-9) * std::pow(T, 3);
    } else {
        double deltaT = T - Tm;
...
double Material::getRho(double T) const {
    ...
    if (T0 <= T && T <= Tm) {
        return conv * (19.25 - (2.66207e-4) * dT - (3.0595e-9) * (dT * dT) -
                       (9.5185e-12) * std::pow(dT, 3));
    } else {
        double dTm = T - Tm;
...
double Material::getCp(double T) const {
    ...
    if (T <= 3080.0) {
        return conv * (21.868372 + (8.068661e-3) * T - (3.756196e-6) * (T * T) +
                       (1.075862e-9) * std::pow(T, 3) + (1.406637e4) / (T * T));
    } else if (T <= 3695.0) {
