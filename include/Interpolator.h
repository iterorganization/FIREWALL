...
class Interpolator {
   public:
    ...
    // mapped to the provided target spatial grid (dx1, dx2 logic handles grid generation externally
    // or internally) The python code generates the grid inside 'interpolate_with_dx'. Here we will
    // return the profile on the target grid provided as argument.
    std::vector<double> getProfile(double energy, double angle,
